...
pass: consultar en local


Personal Access Tokens: consultar en local
Personal access Token Cubing 2025: consultar en local  
# Si generaste un nuevo token actualiza la url del remote:
git remote set-url origin https://marcelo.alarcon:consultar en local@gitlab.com/altacrest/x5.xenvio.git
...
Feed token: consultar en local
Incoming email token: consultar en local
# Git Clone
git clone https://marcelo.alarcon:consultar en local@gitlab.com/altacrest/x5.xenvio.git
...
P: consultar en local

account_number: 799909537
API Key: consultar en local
API Secret: consultar en local
...
![PastedGraphics](file:///.attachment/A9B43BB4-FF55-4BDB-9AE9-B96940F86DBB.png)
